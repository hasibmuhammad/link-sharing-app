...
const Links = (): JSX.Element => {
    ...
    const toggleDropdown = (id: number) => {
        setLinks(links.map(link => link.id === id ? { ...link, isDropdownOpen: !link.isDropdownOpen } : link));
    };

    const handlePlatformSelect = (id: number, selectedItem: number) => {
        setLinks(links.map(link => link.id === id ? { ...link, selectedItem, isDropdownOpen: false } : link));
    };
    ...
    return (
        <section className="max-w-[1300px] mx-auto">
            <div className="grid md:grid-cols-3 gap-10">
                ...
                <div className="bg-white p-10 md:col-span-2 rounded-lg">
                    ...
                    <div className="mt-10">
                        <button onClick={handleAddNewLink} className="hover:bg-primary/10 w-full border border-primary rounded-lg font-bold text-primary py-2">+ Add New Link</button>
                    </div>
                    ...
                    {links.map((li) => {
                        const selectedOne = listArray.find(item => item.id === li.selectedItem);
                        return (
